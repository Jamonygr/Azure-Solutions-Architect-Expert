...
BeforeAll {
    ...
    $BaseParameters = [ordered]@{
        ...
        CatalogId = 'synthetic-catalog-id'
        InitiativeName = 'synthetic-initiative-name'
        PolicyAssignmentName = 'synthetic-policy-assignment-name'
        PolicyCategory = 'synthetic-policy-category'
        PolicyScope = 'synthetic-policy-scope'
        ReviewDefinitionId = 'synthetic-review-definition-id'
    }
...
function global:Get-AzPolicyExemption { Record-Az305TestCall -Name 'Get-AzPolicyExemption'; throw 'Injected checkpoint failure.' }
function global:Get-AzPolicySetDefinition { Record-Az305TestCall -Name 'Get-AzPolicySetDefinition'; throw 'Injected checkpoint failure.' }
function global:Get-AzPolicyState { Record-Az305TestCall -Name 'Get-AzPolicyState'; throw 'Injected checkpoint failure.' }
function global:Get-MgEntitlementManagementCatalog { Record-Az305TestCall -Name 'Get-MgEntitlementManagementCatalog'; throw 'Injected checkpoint failure.' }
function global:Get-MgIdentityGovernanceAccessReviewDefinition { Record-Az305TestCall -Name 'Get-MgIdentityGovernanceAccessReviewDefinition'; throw 'Injected checkpoint failure.' }
function global:Get-MgRoleManagementDirectoryRoleAssignmentScheduleInstance { Record-Az305TestCall -Name 'Get-MgRoleManagementDirectoryRoleAssignmentScheduleInstance'; throw 'Injected checkpoint failure.' }
function global:Get-MgRoleManagementDirectoryRoleEligibilitySchedule { Record-Az305TestCall -Name 'Get-MgRoleManagementDirectoryRoleEligibilitySchedule'; throw 'Injected checkpoint failure.' }
...
Describe 'LAB-07 portable lifecycle behavior' {
    BeforeEach {
...
'@
    ...
    function Get-ManagedObjectFixture {
        ...
        return [pscustomobject]@{
            ...
            tags = [ordered]@{ purpose = 'az305-lab'; labId = 'LAB-07'; runId = $RecordedRunId; expiresOn = '2099-12-31' }
        }
    }
...
'@.Replace('__LAB_ID__', 'LAB-07')
    ...
    It 'writes recovery state before an injected checkpoint failure and preserves failed status' {
        if ($IsDesignSimulation) {
            (Get-Content (Join-Path $ScriptRoot 'Setup.ps1') -Raw).IndexOf('Save-RunState -State $state') | Should -BeLessThan (Get-Content (Join-Path $ScriptRoot 'Setup.ps1') -Raw).IndexOf('# 07-CP01:')
            return
        }
        $run = 'failure-000001'
    ...
    It 'executes every positive and negative checkpoint independently or gates design cloud validation' {
        ...
        if ($ValidationHasDesignCloudGate) {
            ...
            @($artifact.assertions | Where-Object id -Match '^LAB07-CP0[1-5]-(POS|NEG)$').Count | Should -Be 10
        }
    }
