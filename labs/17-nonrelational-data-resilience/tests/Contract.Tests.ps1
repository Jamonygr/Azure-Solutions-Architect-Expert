...
BeforeAll {
    ...
    $BaseParameters = [ordered]@{
        ...
        ApprovedBlobRegions = 'synthetic-approved-blob-regions'
        ApprovedCosmosRegions = 'synthetic-approved-cosmos-regions'
        ConflictResolutionProcedureApproved = $false
        CosmosAccountName = 'synthetic-cosmos-account-name'
...
function global:Get-AzCosmosDBAccount { Record-Az305TestCall -Name 'Get-AzCosmosDBAccount'; throw 'Injected checkpoint failure.' }
function global:Get-AzCosmosDBSqlDatabase { Record-Az305TestCall -Name 'Get-AzCosmosDBSqlDatabase'; throw 'Injected checkpoint failure.' }
function global:Get-AzStorageAccount { Record-Az305TestCall -Name 'Get-AzStorageAccount'; throw 'Injected checkpoint failure.' }
function global:Get-AzStorageBlobServiceProperty { Record-Az305TestCall -Name 'Get-AzStorageBlobServiceProperty'; throw 'Injected checkpoint failure.' }
...
Describe 'LAB-17 portable lifecycle behavior' {
    BeforeEach {
...
'@
    ...
    function Get-ManagedObjectFixture {
        ...
        return [pscustomobject]@{
            ...
            tags = [ordered]@{ purpose = 'az305-lab'; labId = 'LAB-17'; runId = $RecordedRunId; expiresOn = '2099-12-31' }
        }
    }
...
'@.Replace('__LAB_ID__', 'LAB-17')
    ...
    It 'writes recovery state before an injected checkpoint failure and preserves failed status' {
        if ($IsDesignSimulation) {
            (Get-Content (Join-Path $ScriptRoot 'Setup.ps1') -Raw).IndexOf('Save-RunState -State $state') | Should -BeLessThan (Get-Content (Join-Path $ScriptRoot 'Setup.ps1') -Raw).IndexOf('# 17-CP01:')
            return
        }
        $run = 'failure-000001'
    ...
    It 'executes every positive and negative checkpoint independently or gates design cloud validation' {
        ...
        if ($ValidationHasDesignCloudGate) {
            ...
            @($artifact.assertions | Where-Object id -Match '^LAB17-CP0[1-5]-(POS|NEG)$').Count | Should -Be 10
        }
    }
