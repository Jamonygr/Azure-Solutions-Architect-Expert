...
BeforeAll {
    ...
    $BaseParameters = [ordered]@{
        ...
        ArcServerResourceId = '/subscriptions/00000000-0000-4000-8000-000000000305/resourceGroups/rg-az305-contract/providers/Microsoft.Test/resources/synthetic'
        CustomRoleName = 'synthetic-custom-role-name'
        GroupObjectId = '00000000-0000-4000-8000-000000000307'
        PrincipalObjectId = '00000000-0000-4000-8000-000000000307'
        ResourceGroupId = 'synthetic-resource-group-id'
...
Describe 'LAB-04 portable lifecycle behavior' {
    BeforeEach {
...
'@
    ...
    function Get-ManagedObjectFixture {
        ...
        return [pscustomobject]@{
            ...
            tags = [ordered]@{ purpose = 'az305-lab'; labId = 'LAB-04'; runId = $RecordedRunId; expiresOn = '2099-12-31' }
        }
    }
...
'@.Replace('__LAB_ID__', 'LAB-04')
    ...
    It 'writes recovery state before an injected checkpoint failure and preserves failed status' {
        if ($IsDesignSimulation) {
            (Get-Content (Join-Path $ScriptRoot 'Setup.ps1') -Raw).IndexOf('Save-RunState -State $state') | Should -BeLessThan (Get-Content (Join-Path $ScriptRoot 'Setup.ps1') -Raw).IndexOf('# 04-CP01:')
            return
        }
        $run = 'failure-000001'
    ...
    It 'executes every positive and negative checkpoint independently or gates design cloud validation' {
        ...
        if ($ValidationHasDesignCloudGate) {
            ...
            @($artifact.assertions | Where-Object id -Match '^LAB04-CP0[1-5]-(POS|NEG)$').Count | Should -Be 10
        }
    }
