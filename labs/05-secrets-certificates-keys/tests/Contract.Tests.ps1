...
BeforeAll {
    ...
    $BaseParameters = [ordered]@{
        ...
        CertificateName = 'synthetic-certificate-name'
        KeyName = 'synthetic-key-name'
        VaultName = 'synthetic-vault-name'
        VaultResourceId = '/subscriptions/00000000-0000-4000-8000-000000000305/resourceGroups/rg-az305-contract/providers/Microsoft.Test/resources/synthetic'
        WorkloadPrincipalId = '00000000-0000-4000-8000-000000000307'
    }
...
function global:Get-AzKeyVault { Record-Az305TestCall -Name 'Get-AzKeyVault'; throw 'Injected checkpoint failure.' }
function global:Get-AzKeyVaultCertificate { Record-Az305TestCall -Name 'Get-AzKeyVaultCertificate'; throw 'Injected checkpoint failure.' }
function global:Get-AzKeyVaultCertificatePolicy { Record-Az305TestCall -Name 'Get-AzKeyVaultCertificatePolicy'; throw 'Injected checkpoint failure.' }
function global:Get-AzKeyVaultKey { Record-Az305TestCall -Name 'Get-AzKeyVaultKey'; throw 'Injected checkpoint failure.' }
function global:Get-AzKeyVaultKeyRotationPolicy { Record-Az305TestCall -Name 'Get-AzKeyVaultKeyRotationPolicy'; throw 'Injected checkpoint failure.' }
function global:Get-AzRoleAssignment { Record-Az305TestCall -Name 'Get-AzRoleAssignment'; throw 'Injected checkpoint failure.' }
function global:New-AzKeyVault { Record-Az305TestCall -Name 'New-AzKeyVault'; throw 'Injected checkpoint failure.' }
function global:Update-AzKeyVault { Record-Az305TestCall -Name 'Update-AzKeyVault'; throw 'Injected checkpoint failure.' }
...
Describe 'LAB-05 portable lifecycle behavior' {
    BeforeEach {
...
'@
    ...
    function Get-ManagedObjectFixture {
        ...
        return [pscustomobject]@{
            ...
            tags = [ordered]@{ purpose = 'az305-lab'; labId = 'LAB-05'; runId = $RecordedRunId; expiresOn = '2099-12-31' }
        }
    }
...
'@.Replace('__LAB_ID__', 'LAB-05')
    ...
    It 'writes recovery state before an injected checkpoint failure and preserves failed status' {
        if ($IsDesignSimulation) {
            (Get-Content (Join-Path $ScriptRoot 'Setup.ps1') -Raw).IndexOf('Save-RunState -State $state') | Should -BeLessThan (Get-Content (Join-Path $ScriptRoot 'Setup.ps1') -Raw).IndexOf('# 05-CP01:')
            return
        }
        $run = 'failure-000001'
    ...
    It 'executes every positive and negative checkpoint independently or gates design cloud validation' {
        ...
        if ($ValidationHasDesignCloudGate) {
            ...
            @($artifact.assertions | Where-Object id -Match '^LAB05-CP0[1-5]-(POS|NEG)$').Count | Should -Be 10
        }
    }
