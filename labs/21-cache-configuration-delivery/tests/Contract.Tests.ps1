...
BeforeAll {
    ...
    $BaseParameters = [ordered]@{
        ...
        AppConfigurationName = 'synthetic-app-configuration-name'
        ManagedRedisResourceId = '/subscriptions/00000000-0000-4000-8000-000000000305/resourceGroups/rg-az305-contract/providers/Microsoft.Test/resources/synthetic'
...
function global:Get-AzAppConfigurationStore { Record-Az305TestCall -Name 'Get-AzAppConfigurationStore'; throw 'Injected checkpoint failure.' }
function global:Get-AzMetricAlertRuleV2 { Record-Az305TestCall -Name 'Get-AzMetricAlertRuleV2'; throw 'Injected checkpoint failure.' }
...
function global:Get-AzResource { Record-Az305TestCall -Name 'Get-AzResource'; throw 'Injected checkpoint failure.' }
function global:Get-AzResourceGroupDeployment { Record-Az305TestCall -Name 'Get-AzResourceGroupDeployment'; throw 'Injected checkpoint failure.' }
function global:New-AzResourceGroupDeployment { Record-Az305TestCall -Name 'New-AzResourceGroupDeployment'; throw 'Injected checkpoint failure.' }
function global:Test-AzResourceGroupDeployment { Record-Az305TestCall -Name 'Test-AzResourceGroupDeployment'; throw 'Injected checkpoint failure.' }
...
Describe 'LAB-21 portable lifecycle behavior' {
    BeforeEach {
...
'@
    ...
    function Get-ManagedObjectFixture {
        ...
        return [pscustomobject]@{
            ...
            tags = [ordered]@{ purpose = 'az305-lab'; labId = 'LAB-21'; runId = $RecordedRunId; expiresOn = '2099-12-31' }
        }
    }
...
'@.Replace('__LAB_ID__', 'LAB-21')
    ...
    It 'writes recovery state before an injected checkpoint failure and preserves failed status' {
        if ($IsDesignSimulation) {
            (Get-Content (Join-Path $ScriptRoot 'Setup.ps1') -Raw).IndexOf('Save-RunState -State $state') | Should -BeLessThan (Get-Content (Join-Path $ScriptRoot 'Setup.ps1') -Raw).IndexOf('# 21-CP01:')
            return
        }
        $run = 'failure-000001'
    ...
    It 'executes every positive and negative checkpoint independently or gates design cloud validation' {
        ...
        if ($ValidationHasDesignCloudGate) {
            ...
            @($artifact.assertions | Where-Object id -Match '^LAB21-CP0[1-5]-(POS|NEG)$').Count | Should -Be 10
        }
    }
