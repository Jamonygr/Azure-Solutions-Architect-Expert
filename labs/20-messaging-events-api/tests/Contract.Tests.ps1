...
BeforeAll {
    ...
    $BaseParameters = [ordered]@{
        ...
        ApiManagementName = 'synthetic-api-management-name'
        CommandQueueName = 'synthetic-command-queue-name'
        EventHubNamespace = 'synthetic-event-hub-namespace'
        EventTopicResourceId = '/subscriptions/00000000-0000-4000-8000-000000000305/resourceGroups/rg-az305-contract/providers/Microsoft.Test/resources/synthetic'
        PartnerApiId = 'synthetic-partner-api-id'
        ResourceGroupName = 'synthetic-resource-group-name'
        ServiceBusNamespace = 'synthetic-service-bus-namespace'
        ServiceBusNamespaceResourceId = '/subscriptions/00000000-0000-4000-8000-000000000305/resourceGroups/rg-az305-contract/providers/Microsoft.Test/resources/synthetic'
        TelemetryHubName = 'synthetic-telemetry-hub-name'
    }
...
Describe 'LAB-20 portable lifecycle behavior' {
    BeforeEach {
...
'@
    ...
    function Get-ManagedObjectFixture {
        ...
        return [pscustomobject]@{
            ...
            tags = [ordered]@{ purpose = 'az305-lab'; labId = 'LAB-20'; runId = $RecordedRunId; expiresOn = '2099-12-31' }
        }
    }
...
'@.Replace('__LAB_ID__', 'LAB-20')
    ...
    It 'writes recovery state before an injected checkpoint failure and preserves failed status' {
        if ($IsDesignSimulation) {
            (Get-Content (Join-Path $ScriptRoot 'Setup.ps1') -Raw).IndexOf('Save-RunState -State $state') | Should -BeLessThan (Get-Content (Join-Path $ScriptRoot 'Setup.ps1') -Raw).IndexOf('# 20-CP01:')
            return
        }
        $run = 'failure-000001'
    ...
    It 'executes every positive and negative checkpoint independently or gates design cloud validation' {
        ...
        if ($ValidationHasDesignCloudGate) {
            ...
            @($artifact.assertions | Where-Object id -Match '^LAB20-CP0[1-5]-(POS|NEG)$').Count | Should -Be 10
        }
    }
