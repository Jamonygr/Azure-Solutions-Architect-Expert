...
BeforeAll {
    ...
    $BaseParameters = [ordered]@{
        ...
        RecoveryPlanName = 'synthetic-recovery-plan-name'
        ResourceGroupName = 'synthetic-resource-group-name'
        VaultId = 'synthetic-vault-id'
        VmSku = 'synthetic-vm-sku'
        WebTierVmName = 'synthetic-web-tier-vm-name'
    }
...
function global:Get-AzComputeResourceSku { Record-Az305TestCall -Name 'Get-AzComputeResourceSku'; throw 'Injected checkpoint failure.' }
function global:Get-AzRecoveryServicesAsrRecoveryPlan { Record-Az305TestCall -Name 'Get-AzRecoveryServicesAsrRecoveryPlan'; throw 'Injected checkpoint failure.' }
function global:Get-AzRecoveryServicesBackupJob { Record-Az305TestCall -Name 'Get-AzRecoveryServicesBackupJob'; throw 'Injected checkpoint failure.' }
function global:Get-AzRecoveryServicesVault { Record-Az305TestCall -Name 'Get-AzRecoveryServicesVault'; throw 'Injected checkpoint failure.' }
function global:Get-AzVM { Record-Az305TestCall -Name 'Get-AzVM'; throw 'Injected checkpoint failure.' }
...
Describe 'LAB-15 portable lifecycle behavior' {
    BeforeEach {
...
'@
    ...
    function Get-ManagedObjectFixture {
        ...
        return [pscustomobject]@{
            ...
            tags = [ordered]@{ purpose = 'az305-lab'; labId = 'LAB-15'; runId = $RecordedRunId; expiresOn = '2099-12-31' }
        }
    }
...
'@.Replace('__LAB_ID__', 'LAB-15')
    ...
    It 'writes recovery state before an injected checkpoint failure and preserves failed status' {
        if ($IsDesignSimulation) {
            (Get-Content (Join-Path $ScriptRoot 'Setup.ps1') -Raw).IndexOf('Save-RunState -State $state') | Should -BeLessThan (Get-Content (Join-Path $ScriptRoot 'Setup.ps1') -Raw).IndexOf('# 15-CP01:')
            return
        }
        $run = 'failure-000001'
    ...
    It 'executes every positive and negative checkpoint independently or gates design cloud validation' {
        ...
        if ($ValidationHasDesignCloudGate) {
            ...
            @($artifact.assertions | Where-Object id -Match '^LAB15-CP0[1-5]-(POS|NEG)$').Count | Should -Be 10
        }
    }
