...
BeforeAll {
    ...
    $BaseParameters = [ordered]@{
        ...
        ApplicationSubnetName = 'synthetic-application-subnet-name'
        ApprovedDestinationIp = '192.0.2.10'
        ApprovedFirewallEgressRoute = $false
        DestinationIp = '192.0.2.10'
        DisallowedDestinationIp = '192.0.2.10'
        FirewallName = 'synthetic-firewall-name'
        FirewallPolicyName = 'synthetic-firewall-policy-name'
        FrontDoorProfileName = 'synthetic-front-door-profile-name'
        InternalLoadBalancerName = 'synthetic-internal-load-balancer-name'
        NetworkWatcherName = 'synthetic-network-watcher-name'
        ResourceGroupName = 'synthetic-resource-group-name'
        SourceIp = '192.0.2.10'
        SourceVmId = '/subscriptions/00000000-0000-4000-8000-000000000305/resourceGroups/rg-az305-contract/providers/Microsoft.Test/resources/synthetic'
        SpokeVnetName = 'synthetic-spoke-vnet-name'
    }
...
function global:Get-AzApplicationGateway { Record-Az305TestCall -Name 'Get-AzApplicationGateway'; throw 'Injected checkpoint failure.' }
function global:Get-AzFirewall { Record-Az305TestCall -Name 'Get-AzFirewall'; throw 'Injected checkpoint failure.' }
function global:Get-AzFirewallPolicy { Record-Az305TestCall -Name 'Get-AzFirewallPolicy'; throw 'Injected checkpoint failure.' }
function global:Get-AzFrontDoorCdnProfile { Record-Az305TestCall -Name 'Get-AzFrontDoorCdnProfile'; throw 'Injected checkpoint failure.' }
function global:Get-AzLoadBalancer { Record-Az305TestCall -Name 'Get-AzLoadBalancer'; throw 'Injected checkpoint failure.' }
function global:Get-AzNetworkSecurityGroup { Record-Az305TestCall -Name 'Get-AzNetworkSecurityGroup'; throw 'Injected checkpoint failure.' }
function global:Get-AzNetworkWatcher { Record-Az305TestCall -Name 'Get-AzNetworkWatcher'; throw 'Injected checkpoint failure.' }
function global:Get-AzVirtualNetwork { Record-Az305TestCall -Name 'Get-AzVirtualNetwork'; throw 'Injected checkpoint failure.' }
function global:Test-AzNetworkWatcherIPFlow { Record-Az305TestCall -Name 'Test-AzNetworkWatcherIPFlow'; throw 'Injected checkpoint failure.' }
...
Describe 'LAB-25 portable lifecycle behavior' {
    BeforeEach {
...
'@
    ...
    function Write-TestRunState {
        ...
        $state = [ordered]@{
            ...
            implementationMode = 'safe-analogue'; status = $Status
    ...
    function Get-ManagedObjectFixture {
        ...
        return [pscustomobject]@{
            ...
            tags = [ordered]@{ purpose = 'az305-lab'; labId = 'LAB-25'; runId = $RecordedRunId; expiresOn = '2099-12-31' }
        }
    }
...
'@.Replace('__LAB_ID__', 'LAB-25')
    ...
    It 'writes recovery state before an injected checkpoint failure and preserves failed status' {
        if ($IsDesignSimulation) {
            (Get-Content (Join-Path $ScriptRoot 'Setup.ps1') -Raw).IndexOf('Save-RunState -State $state') | Should -BeLessThan (Get-Content (Join-Path $ScriptRoot 'Setup.ps1') -Raw).IndexOf('# 25-CP01:')
            return
        }
        $run = 'failure-000001'
    ...
    It 'executes every positive and negative checkpoint independently or gates design cloud validation' {
        ...
        if ($ValidationHasDesignCloudGate) {
            ...
            @($artifact.assertions | Where-Object id -Match '^LAB25-CP0[1-5]-(POS|NEG)$').Count | Should -Be 10
        }
    }
