...
BeforeAll {
    ...
    $BaseParameters = [ordered]@{
        ...
        DataFactoryName = 'synthetic-data-factory-name'
        IntegrationRuntimeName = 'synthetic-integration-runtime-name'
        LinkedServiceName = 'synthetic-linked-service-name'
        PipelineRunId = 'synthetic-pipeline-run-id'
        RunStartedAfter = 'synthetic-run-started-after'
        RunStartedBefore = 'synthetic-run-started-before'
        SynapseWorkspaceName = 'synthetic-synapse-workspace-name'
    }
...
function global:Get-AzDataFactoryV2ActivityRun { Record-Az305TestCall -Name 'Get-AzDataFactoryV2ActivityRun'; throw 'Injected checkpoint failure.' }
function global:Get-AzDataFactoryV2IntegrationRuntime { Record-Az305TestCall -Name 'Get-AzDataFactoryV2IntegrationRuntime'; throw 'Injected checkpoint failure.' }
function global:Get-AzDataFactoryV2LinkedService { Record-Az305TestCall -Name 'Get-AzDataFactoryV2LinkedService'; throw 'Injected checkpoint failure.' }
function global:Get-AzDataFactoryV2PipelineRun { Record-Az305TestCall -Name 'Get-AzDataFactoryV2PipelineRun'; throw 'Injected checkpoint failure.' }
function global:Get-AzSynapseSqlPool { Record-Az305TestCall -Name 'Get-AzSynapseSqlPool'; throw 'Injected checkpoint failure.' }
function global:Get-AzSynapseWorkspace { Record-Az305TestCall -Name 'Get-AzSynapseWorkspace'; throw 'Injected checkpoint failure.' }
...
Describe 'LAB-13 portable lifecycle behavior' {
    BeforeEach {
...
'@
    ...
    function Get-ManagedObjectFixture {
        ...
        return [pscustomobject]@{
            ...
            tags = [ordered]@{ purpose = 'az305-lab'; labId = 'LAB-13'; runId = $RecordedRunId; expiresOn = '2099-12-31' }
        }
    }
...
'@.Replace('__LAB_ID__', 'LAB-13')
    ...
    It 'writes recovery state before an injected checkpoint failure and preserves failed status' {
        if ($IsDesignSimulation) {
            (Get-Content (Join-Path $ScriptRoot 'Setup.ps1') -Raw).IndexOf('Save-RunState -State $state') | Should -BeLessThan (Get-Content (Join-Path $ScriptRoot 'Setup.ps1') -Raw).IndexOf('# 13-CP01:')
            return
        }
        $run = 'failure-000001'
    ...
    It 'executes every positive and negative checkpoint independently or gates design cloud validation' {
        ...
        if ($ValidationHasDesignCloudGate) {
            ...
            @($artifact.assertions | Where-Object id -Match '^LAB13-CP0[1-5]-(POS|NEG)$').Count | Should -Be 10
        }
    }
