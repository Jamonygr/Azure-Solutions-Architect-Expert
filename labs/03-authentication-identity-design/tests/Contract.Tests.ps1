...
BeforeAll {
    ...
    $BaseParameters = [ordered]@{
        ...
        AppDisplayName = 'synthetic-app-display-name'
        WorkloadIdentityName = 'synthetic-workload-identity-name'
    }
...
function global:Get-MgApplication { Record-Az305TestCall -Name 'Get-MgApplication'; throw 'Injected checkpoint failure.' }
function global:Get-MgIdentityConditionalAccessPolicy { Record-Az305TestCall -Name 'Get-MgIdentityConditionalAccessPolicy'; throw 'Injected checkpoint failure.' }
function global:Get-MgOrganization { Record-Az305TestCall -Name 'Get-MgOrganization'; throw 'Injected checkpoint failure.' }
function global:Get-MgPolicyAuthenticationMethodPolicy { Record-Az305TestCall -Name 'Get-MgPolicyAuthenticationMethodPolicy'; throw 'Injected checkpoint failure.' }
function global:Get-MgPolicyAuthenticationMethodPolicyAuthenticationMethodConfiguration { Record-Az305TestCall -Name 'Get-MgPolicyAuthenticationMethodPolicyAuthenticationMethodConfiguration'; throw 'Injected checkpoint failure.' }
function global:Get-MgServicePrincipal { Record-Az305TestCall -Name 'Get-MgServicePrincipal'; throw 'Injected checkpoint failure.' }
...
Describe 'LAB-03 portable lifecycle behavior' {
    BeforeEach {
...
'@
    ...
    function Get-ManagedObjectFixture {
        ...
        return [pscustomobject]@{
            ...
            tags = [ordered]@{ purpose = 'az305-lab'; labId = 'LAB-03'; runId = $RecordedRunId; expiresOn = '2099-12-31' }
        }
    }
...
'@.Replace('__LAB_ID__', 'LAB-03')
    ...
    It 'writes recovery state before an injected checkpoint failure and preserves failed status' {
        if ($IsDesignSimulation) {
            (Get-Content (Join-Path $ScriptRoot 'Setup.ps1') -Raw).IndexOf('Save-RunState -State $state') | Should -BeLessThan (Get-Content (Join-Path $ScriptRoot 'Setup.ps1') -Raw).IndexOf('# 03-CP01:')
            return
        }
        $run = 'failure-000001'
    ...
    It 'executes every positive and negative checkpoint independently or gates design cloud validation' {
        ...
        if ($ValidationHasDesignCloudGate) {
            ...
            @($artifact.assertions | Where-Object id -Match '^LAB03-CP0[1-5]-(POS|NEG)$').Count | Should -Be 10
        }
    }
