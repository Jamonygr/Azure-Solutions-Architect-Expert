...
BeforeAll {
    ...
    $BaseParameters = [ordered]@{
        ...
        ApprovedReplicaCount = 1
        ApprovedShortTermRetentionDays = 1
        ApprovedVCoreCeiling = 1
        DatabaseName = 'synthetic-database-name'
        SqlServerName = 'synthetic-sql-server-name'
    }
...
function global:Get-AzSqlDatabase { Record-Az305TestCall -Name 'Get-AzSqlDatabase'; throw 'Injected checkpoint failure.' }
function global:Get-AzSqlDatabaseAudit { Record-Az305TestCall -Name 'Get-AzSqlDatabaseAudit'; throw 'Injected checkpoint failure.' }
function global:Get-AzSqlDatabaseBackupLongTermRetentionPolicy { Record-Az305TestCall -Name 'Get-AzSqlDatabaseBackupLongTermRetentionPolicy'; throw 'Injected checkpoint failure.' }
function global:Get-AzSqlDatabaseBackupShortTermRetentionPolicy { Record-Az305TestCall -Name 'Get-AzSqlDatabaseBackupShortTermRetentionPolicy'; throw 'Injected checkpoint failure.' }
function global:Get-AzSqlDatabaseTransparentDataEncryption { Record-Az305TestCall -Name 'Get-AzSqlDatabaseTransparentDataEncryption'; throw 'Injected checkpoint failure.' }
function global:Get-AzSqlServerKeyVaultKey { Record-Az305TestCall -Name 'Get-AzSqlServerKeyVaultKey'; throw 'Injected checkpoint failure.' }
...
Describe 'LAB-09 portable lifecycle behavior' {
    BeforeEach {
...
'@
    ...
    function Get-ManagedObjectFixture {
        ...
        return [pscustomobject]@{
            ...
            tags = [ordered]@{ purpose = 'az305-lab'; labId = 'LAB-09'; runId = $RecordedRunId; expiresOn = '2099-12-31' }
        }
    }
...
'@.Replace('__LAB_ID__', 'LAB-09')
    ...
    It 'writes recovery state before an injected checkpoint failure and preserves failed status' {
        if ($IsDesignSimulation) {
            (Get-Content (Join-Path $ScriptRoot 'Setup.ps1') -Raw).IndexOf('Save-RunState -State $state') | Should -BeLessThan (Get-Content (Join-Path $ScriptRoot 'Setup.ps1') -Raw).IndexOf('# 09-CP01:')
            return
        }
        $run = 'failure-000001'
    ...
    It 'executes every positive and negative checkpoint independently or gates design cloud validation' {
        ...
        if ($ValidationHasDesignCloudGate) {
            ...
            @($artifact.assertions | Where-Object id -Match '^LAB09-CP0[1-5]-(POS|NEG)$').Count | Should -Be 10
        }
    }
