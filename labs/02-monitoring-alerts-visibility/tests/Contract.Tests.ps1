...
BeforeAll {
    ...
    $IsDesignSimulation = $false
    $IsFoundationPreview = $false
    $SetupHasDesignCloudGate = $false
    $ValidationHasDesignCloudGate = $false
    $IsCliTrack = $false
    ...
    $BaseParameters = [ordered]@{
        ...
        ActionGroupName = 'synthetic-action-group-name'
        ActionGroupResourceId = '/subscriptions/00000000-0000-4000-8000-000000000305/resourceGroups/rg-az305-contract/providers/Microsoft.Test/resources/synthetic'
        DeprecatedMetricName = 'synthetic-deprecated-metric-name'
        HealthAlertName = 'synthetic-health-alert-name'
        RequiredMetricName = 'synthetic-required-metric-name'
        ScheduledQueryRuleName = 'synthetic-scheduled-query-rule-name'
        TargetResourceId = '/subscriptions/00000000-0000-4000-8000-000000000305/resourceGroups/rg-az305-contract/providers/Microsoft.Test/resources/synthetic'
        WorkbookDisplayName = 'synthetic-workbook-display-name'
        WorkbookJson = '{}'
        WorkbookResourceName = 'synthetic-workbook-resource-name'
    }
...
function global:Get-AzActionGroup { Record-Az305TestCall -Name 'Get-AzActionGroup'; throw 'Injected checkpoint failure.' }
function global:Get-AzActivityLogAlert { Record-Az305TestCall -Name 'Get-AzActivityLogAlert'; throw 'Injected checkpoint failure.' }
function global:Get-AzApplicationInsightsWorkbook { Record-Az305TestCall -Name 'Get-AzApplicationInsightsWorkbook'; throw 'Injected checkpoint failure.' }
function global:Get-AzMetricDefinition { Record-Az305TestCall -Name 'Get-AzMetricDefinition'; throw 'Injected checkpoint failure.' }
function global:Get-AzScheduledQueryRule { Record-Az305TestCall -Name 'Get-AzScheduledQueryRule'; throw 'Injected checkpoint failure.' }
function global:New-AzActivityLogAlert { Record-Az305TestCall -Name 'New-AzActivityLogAlert'; throw 'Injected checkpoint failure.' }
function global:New-AzActivityLogAlertActionGroupObject { Record-Az305TestCall -Name 'New-AzActivityLogAlertActionGroupObject'; throw 'Injected checkpoint failure.' }
function global:New-AzActivityLogAlertAlertRuleAnyOfOrLeafConditionObject { Record-Az305TestCall -Name 'New-AzActivityLogAlertAlertRuleAnyOfOrLeafConditionObject'; throw 'Injected checkpoint failure.' }
function global:New-AzApplicationInsightsWorkbook { Record-Az305TestCall -Name 'New-AzApplicationInsightsWorkbook'; throw 'Injected checkpoint failure.' }
...
Describe 'LAB-02 portable lifecycle behavior' {
    BeforeEach {
...
'@
    ...
    function Write-TestRunState {
        param([string]$RunId, [string]$Status = 'deployed', [object[]]$ManagedObjects = @(), [object[]]$OriginalSettings = @())
        ...
        $state = [ordered]@{
            ...
            implementationMode = 'reference-deployable'; status = $Status
    ...
    function Get-ManagedObjectFixture {
        ...
        return [pscustomobject]@{
            ...
            tags = [ordered]@{ purpose = 'az305-lab'; labId = 'LAB-02'; runId = $RecordedRunId; expiresOn = '2099-12-31' }
        }
    }
...
'@.Replace('__LAB_ID__', 'LAB-02')
    ...
    It 'writes recovery state before an injected checkpoint failure and preserves failed status' {
        if ($IsDesignSimulation) {
            (Get-Content (Join-Path $ScriptRoot 'Setup.ps1') -Raw).IndexOf('Save-RunState -State $state') | Should -BeLessThan (Get-Content (Join-Path $ScriptRoot 'Setup.ps1') -Raw).IndexOf('# 02-CP01:')
            return
        }
        $run = 'failure-000001'
    ...
    It 'executes every positive and negative checkpoint independently or gates design cloud validation' {
        ...
        if ($ValidationHasDesignCloudGate) {
            ...
            @($artifact.assertions | Where-Object id -Match '^LAB02-CP0[1-5]-(POS|NEG)$').Count | Should -Be 10
        }
    }
