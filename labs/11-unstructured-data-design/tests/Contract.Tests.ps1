...
BeforeAll {
    ...
    $BaseParameters = [ordered]@{
        ...
        DirectoryPath = 'synthetic-directory-path'
        FileSystemName = 'synthetic-file-system-name'
        StorageAccountName = 'synthetic-storage-account-name'
        StorageAccountResourceId = '/subscriptions/00000000-0000-4000-8000-000000000305/resourceGroups/rg-az305-contract/providers/Microsoft.Test/resources/synthetic'
    }
...
function global:Get-AzDataLakeGen2ChildItem { Record-Az305TestCall -Name 'Get-AzDataLakeGen2ChildItem'; throw 'Injected checkpoint failure.' }
function global:Get-AzDataLakeGen2Item { Record-Az305TestCall -Name 'Get-AzDataLakeGen2Item'; throw 'Injected checkpoint failure.' }
function global:Get-AzPrivateEndpointConnection { Record-Az305TestCall -Name 'Get-AzPrivateEndpointConnection'; throw 'Injected checkpoint failure.' }
function global:Get-AzStorageAccount { Record-Az305TestCall -Name 'Get-AzStorageAccount'; throw 'Injected checkpoint failure.' }
function global:Get-AzStorageAccountNameAvailability { Record-Az305TestCall -Name 'Get-AzStorageAccountNameAvailability'; throw 'Injected checkpoint failure.' }
function global:Get-AzStorageAccountNetworkRuleSet { Record-Az305TestCall -Name 'Get-AzStorageAccountNetworkRuleSet'; throw 'Injected checkpoint failure.' }
function global:New-AzStorageAccount { Record-Az305TestCall -Name 'New-AzStorageAccount'; throw 'Injected checkpoint failure.' }
function global:New-AzStorageContainer { Record-Az305TestCall -Name 'New-AzStorageContainer'; throw 'Injected checkpoint failure.' }
...
Describe 'LAB-11 portable lifecycle behavior' {
    BeforeEach {
...
'@
    ...
    function Get-ManagedObjectFixture {
        ...
        return [pscustomobject]@{
            ...
            tags = [ordered]@{ purpose = 'az305-lab'; labId = 'LAB-11'; runId = $RecordedRunId; expiresOn = '2099-12-31' }
        }
    }
...
'@.Replace('__LAB_ID__', 'LAB-11')
    ...
    It 'writes recovery state before an injected checkpoint failure and preserves failed status' {
        if ($IsDesignSimulation) {
            (Get-Content (Join-Path $ScriptRoot 'Setup.ps1') -Raw).IndexOf('Save-RunState -State $state') | Should -BeLessThan (Get-Content (Join-Path $ScriptRoot 'Setup.ps1') -Raw).IndexOf('# 11-CP01:')
            return
        }
        $run = 'failure-000001'
    ...
    It 'executes every positive and negative checkpoint independently or gates design cloud validation' {
        ...
        if ($ValidationHasDesignCloudGate) {
            ...
            @($artifact.assertions | Where-Object id -Match '^LAB11-CP0[1-5]-(POS|NEG)$').Count | Should -Be 10
        }
    }
